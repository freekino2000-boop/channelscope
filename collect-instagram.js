/**
 * collect-instagram.js
 * 한국 관련 해시태그로 인스타그램 크리에이터를 찾아 채널 단위 통계(팔로워/팔로잉/게시물수)만 수집합니다.
 * 검색은 로그인이 필요하지만 해시태그 탐색 페이지와 프로필 페이지는 로그인 없이 열린다(직접 확인).
 *
 * 실행: node collect-instagram.js
 *   TARGET_ADD=500 node collect-instagram.js
 */
const fs = require('fs');
const path = require('path');
const { withBrowser, discoverUsers, getUserProfile } = require('./scraper-instagram');

const POOL_PATH = path.join(__dirname, 'data', 'pool-instagram.json');
...
const SUFFIXES = ['일상', '인플루언서', '추천', '그램', '스타그램'];
...
async function run() {
  ...
  console.log(`[인스타수집] 현재 ${originalCount}개, 추가 목표 ${TARGET_ADD}개, 후보 해시태그 ${queries.length}개`);
  if (!queries.length) { console.log('[인스타수집] 새 해시태그가 없습니다.'); return; }
    ...
    await withBrowser(async (context) => {
      ...
      async function worker() {
        while (batchIdx < batchEnd && newIds.length < TARGET_ADD) {
          ...
          let handles = [];
          try { handles = await discoverUsers(context, query); } catch { handles = []; }
          ...
          if (processed % SAVE_EVERY === 0 || newIds.length >= TARGET_ADD) {
            ...
            save(pool);
            console.log(`[인스타수집] 해시태그 ${processed}/${queries.length}, 새 크리에이터 ${newIds.length}/${TARGET_ADD}, 누적 ${found.size}개`);
          }
        }
      }
  ...
  pool.updatedAt = Date.now();
  save(pool);
  console.log(`[인스타수집] 완료: 기존 ${originalCount}개 → 현재 ${found.size}개 (+${found.size - originalCount}개, 전부 국내 추정)`);
}

run().then(() => process.exit(0)).catch((e) => { console.error('[인스타수집] 오류:', e.message); process.exit(1); });
