...
// 공개본은 원본 history 배열 전체 대신 최대 8개로 솎아낸 경량 버전만 포함(용량 절약)
function miniHistory(history) {
  if (!Array.isArray(history) || history.length < 2) return [];
  if (history.length <= 8) return history.map((p) => ({ t: p.t, s: p.s }));
  const step = (history.length - 1) / 7;
  const picked = [];
  for (let i = 0; i < 8; i++) picked.push(history[Math.round(i * step)]);
  return picked.map((p) => ({ t: p.t, s: p.s }));
}
...
console.log('채널:', channels.length, '| 틱톡:', tiktokCreators.length, '명 | 메타:', facebookCreators.length, '명 | 인스타그램:', instagramCreators.length, '명');
